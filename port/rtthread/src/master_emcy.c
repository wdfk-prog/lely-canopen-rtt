...
/** @brief Return the previous non-zero 32-bit event sequence. */
static rt_uint32_t
        ...
        struct lely_rtt_emcy_event *event)
{
    ...
    for (;;) {
        ...
        while (remaining--) {
            ...
            /*
             * A stable sequence mismatch means the owner wrapped this ring
             * slot after our latest snapshot. Restart after yielding so the
             * owner can publish its new latest sequence; treating the mismatch
             * as "not found" can return -RT_EBUSY with a retained newer EMCY.
             */
