...
lely_rtt_master_command_dispatch(struct lely_rtt_runtime *runtime)
{
    ...
    while (count < PKG_LELY_MASTER_COMMAND_QUEUE_DEPTH) {
        received = rt_mq_recv(runtime->command_mq, &command, sizeof(command),
                RT_WAITING_NO);
        if (received < 0)
            break;
        ...
        count++;
        /*
         * RT-Thread 4.x returns RT_EOK after a successful receive, while 5.3
         * returns the received byte count. Accept both contracts so one dequeued
         * command is never mistaken for an error and silently discarded.
         */
        if (received != RT_EOK
                && (rt_size_t)received != sizeof(command)) {
            LELY_RTT_LOG_W("invalid Master command size: %d", (int)received);
            continue;
        }
...
lely_rtt_master_command_fini(struct lely_rtt_runtime *runtime)
{
    ...
    if (runtime->command_mq) {
        while ((received = rt_mq_recv(runtime->command_mq, &command,
                sizeof(command), RT_WAITING_NO)) >= 0) {
            if (received != RT_EOK
                    && (rt_size_t)received != sizeof(command))
                continue;
